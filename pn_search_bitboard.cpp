...
class pn_node{
    ...
    public:
        ...
        pn_node(bitset<M*N> whites , bitset<M*N> blacks){
            game = new Game(whites , blacks);
        }
    ...
    for(int i = 0 ; i < N ; i++){
        for(int j = 0 ; j < N ; j++){
            if(game->isValidMove(i,j)){
                ...
                pn_node* child = new pn_node(game->whites , game->blacks);
...
void pn_node::evaluate_node(){
    int score = game->evaluate(polyaminos);
...
int main(){
    ...
    if(newfile.is_open()){
        ...
        while(getline(newfile , temp)){
            if(temp.length() == N*N){
                pair<bitset<M*N>,bitset<M*N>>* Pair = make_board_from_file(temp);
                pn_node* root_mobile = new pn_node(Pair->first , Pair->second);
                ...
                pn_node* root_immobile = new pn_node(Pair->first , Pair->second);
